...
class CipherHelperTest : Assert() {
    ...
    fun testGetPoint() {
        ...
        // Validate
        assertArrayEquals(x, removeLeadingZeros(public.w.affineX.toByteArray()))
        assertArrayEquals(y, removeLeadingZeros(public.w.affineY.toByteArray()))
    }
    ...
    fun testVerifyData_success() {
        ...
        val pair = generator.generateKeyPair()
        val signer = Signature.getInstance("SHA224withECDSA").apply {
        ...
        val status = verifyData(dataBase64, signature, pair.public)
    ...
    fun testVerifyData_bad_signature() {
        ...
        val status = verifyData(dataBase64, signatureBase64, pair.public)
    ...
    fun testDecryptAES() {
        ...
        val result = decryptAES(key, ivBase64, encrypted)
    ...
    fun testEncryptAES() {
        ...
        // Testing ...
        val ivBase64 = Base64.encodeToString(iv, Base64.NO_WRAP)
        val result = encryptAES(key, ivBase64, data)
    ...
    fun testCalculateHMAC() {
        ...
        val result = calculateHMAC(key, message)
    ...
    fun testRandomBase64String() {
        val result = randomBase64String(36)

        // Verifying
        assertEquals(result.length, 4 * (36 / 3))
