...
val NAVIGATION_ACTIONS_MAP = hashMapOf(
        ...
        FROM_AUTH_REGISTER_TO_PEOPLE_FRAGMENT to R.id.action_authRegisterFragment_to_peopleFragment
)
...
class AppNavigationManager : NavigationManager {
    ...
    override fun doAction(navController: NavController, id: Int) {
        ...
        if (id == FROM_AUTH_REGISTER_TO_PEOPLE_FRAGMENT) {
            options.setPopUpTo(R.id.navigation_main, true)
        }

        navController.navigate(NAVIGATION_ACTIONS_MAP[id]!!, null, options.build())
    }
