...
class ContactsRepository(private val protocolClient: ProtocolClient,
        ...
        protocolClient.listenMessage("contacts.add", object : ResponseCallback<ContactsDTO> {
            override fun onMessage(response: ContactsDTO)  {
                if(response.checkAvatar)
                    contactsDao.insertContact(Contact(response.id, response.avatarJson,
                            null, response.name, response.nickname))
                else contactsDao.insertContact(Contact(response.id, null, response.avatarUrl, response.name, response.nickname))
    ...
    fun getAllContacts(): MutableLiveData<List<Contact>> {
        ...
        mutableLiveData.postValue(contactsDao.getAllContacts())
        return mutableLiveData
    }
}