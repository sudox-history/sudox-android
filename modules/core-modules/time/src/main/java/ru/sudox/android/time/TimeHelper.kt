...
fun timestampToTimeString(
    ...
    return if (twelveHoursFormat) {
        val amPm = context.getString(
            if (dateTime[ChronoField.AMPM_OF_DAY] == 1) {
                R.string.pm
            } else {
                R.string.am
            }
...
fun timestampToString(
    ...
    if (minutesDiff == 0L) {
        ...
        return formatter.onEventOccurredBetween1And60MinutesAgo(
            context,
            minutesDiff.toInt(),
            requestDateTime,
            relativeDateTime,
    ...
    if (hoursDiff in 1..12) {
        return formatter.onEventOccurredBetween1And12HoursAgo(
            context,
            hoursDiff.toInt(),
            requestDateTime,
            relativeDateTime,
        ...
        return formatter.onEventOccurredBetween12And24HoursAgo(
            context,
            hoursDiff.toInt(),
            requestDateTime,
            relativeDateTime,
    ...
    if (daysDiff == 1L) {
        ...
        return formatter.onEventOccurredBetween2And7DaysAgo(
            context,
            daysDiff.toInt(),
            requestDateTime,
            relativeDateTime,
