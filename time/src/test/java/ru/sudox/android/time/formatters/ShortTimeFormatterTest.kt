...
class ShortTimeFormatterTest {
    ...
    fun testWhenEventOccurredNow() {
        assertEquals("now", onEventOccurredNow(activity, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredBetween1And60MinutesAgo() {
        assertEquals("1m", onEventOccurredBetween1And60MinutesAgo(activity, 1, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredBetween1And12HoursAgo() {
        assertEquals("1h", onEventOccurredBetween1And12HoursAgo(activity, 1, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredBetween12And24HoursAgo() {
        assertEquals("13h", onEventOccurredBetween12And24HoursAgo(activity, 13, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredYesterday() {
        assertEquals("yesterday", onEventOccurredYesterday(activity, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredBetween2And7DaysAgo() {
        assertEquals("thu", onEventOccurredBetween2And7DaysAgo(activity, 2, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredInSameYear() {
        assertEquals("1 jan", onEventOccurredInSameYear(activity, dateTimeOf(0L), dateTimeOf(0L)))
    }

    @Test
    fun testWhenEventOccurredInAnotherYear() {
        assertEquals("1y", onEventOccurredInAnotherYear(activity, 1, dateTimeOf(-31536000L), dateTimeOf(0L)))
    }
}